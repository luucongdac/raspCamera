...
#define  servopinx   11   // digital pin for servo x                     pin kỹ thuật số cho servo x
#define  servopiny   12  // digital servo for pin y                      servo kỹ thuật số cho pin y
...
void pid_servo()
{
  if (Serial3.available() >= 2)  
  {
    ...
    if((d[0] >= 1)&& (valx <= 7))
    {
      ...
      posx = constrain(posx + OutputX, 0, servomaxx);
      posy = constrain(posy + OutputY, 0, servomaxy);
      servox.write(posx);
      servoy.write(posy);
      piControl();
    }
...
//    BROBOT_moveServo2(SERVO2_NEUTRO - 100);
  ...
  // servo initialization            khởi tạo servo
